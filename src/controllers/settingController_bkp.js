...
export const addNewSetting_dt = async (req, res) => {
    ...
    Object.keys(contacts).forEach(async (key, index) => {
        ...
        try {
            ...
            customerAndCallbackRecords.push(customerRecord);
        ...
        if (index == lengthOfContacts - 1) {
            ...
            let dataForUploadContacts = {
                ...
                "customerAndCallbackRecords": customerAndCallbackRecords,
                "properties": {
            ...
            //dataForUploadContacts = JSON.stringify(dataForUploadContacts);
            console.log(dataForUploadContacts);
            let baseURL =  global.gConfig.application_base_url;

            const getData = async (baseURL, dataForUploadContacts) => {
                try {
                    var bodyFormData = new FormData();
                    bodyFormData.append('data',JSON.stringify(dataForUploadContacts));

                    const response = await axios({
                            method: 'post',
                            url: `/ameyowebaccess/command`,
                            baseURL:baseURL,
                            params: {
                                command: 'uploadContactAndAddCallback',
                            ...
                            headers: {
                                ...
                                'host': 'amishra.ameyo.com',
                                'policy-name': 'token-based-authorization-policy',
                                'requesting-host': 'amishra.ameyo.com'
                            },
            ...
            getData(baseURL, dataForUploadContacts);
